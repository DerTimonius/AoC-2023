...
fn parse_plan(input: &str) -> IResult<&str, (Vec<Range<u64>>, Vec<Plan>)> {
  let (input, seeds) = tag("seeds: ")
  .precedes(separated_list1(space1, separated_pair(complete::u64, tag(" "), complete::u64).map(|(start, offset)| start..(start + offset))))
  .parse(input)?;
...
fn do_magic(input: &str) -> u64 {
  ...
  let locations = locations.into_par_iter().map(|seed| {
